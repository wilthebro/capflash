...
5. **Design** — per segment: display mode (word / line / line + highlighted word), font (the bundled Montserrat, a system face, or your own .ttf/.otf/.woff), weight (Normal / Bold / Heavy), size, color, outline color & width, highlight color, and a bounding box you can drag/resize directly on the preview. In line + highlight mode the spoken word is recoloured, bolded and ~10% larger, so it reads at a glance.
   ...
   A segment too long for one line wraps inside its box, and in line/highlight mode the whole block stays on screen for the segment's whole length — the text never jumps or re-breaks mid-sentence, the highlight just travels from word to word (and across lines). Nothing blanks out during a pause either: a word holds until the next one starts. The dashed box is drawn at the height of the captions actually on screen, and a drag is applied when you release the pointer, so the preview stays smooth while you move it.
